...
[[crates/transports/dpdk/src/lib.rs#DpdkL2]] is `BypassTransport<PmdDriver>` from the core [[core#Kernel-bypass shell]], implementing `L2Recv` with backend name `dpdk`. No `Multicast`: the PMD owns the port, so group delivery is arranged on the switch or with the port's multicast filter or all-multicast mode.
...
[[crates/transports/dpdk/src/frame.rs#MbufFrame]] owns exactly one mbuf, caches its data pointer and length so `as_ref` makes no FFI call, and frees the mbuf on drop through the multi-producer mempool put.
...
The bookkeeping test runs on every OS; the EAL tests run on Linux with libdpdk and its pcap PMD.

The in-crate `settle` test pins delivered versus chained counts, one bulk free and the returned count. `tests/real_dpdk.rs` proves the driver links and rejects a null mempool; its ignored tests start one EAL per process (`--no-huge`, `--no-pci`, two `net_pcap` vdevs over pcap files the test writes) and check payload bytes through `UdpDecap` with a wrong-port frame filtered, then an exhausted four-mbuf pool: `no_buffer` rises, `PoolExhausted` never appears, and frames dropped on another thread return every mbuf.
