...
- `tests/tcp.rs`: config rejection before connect, refused connect as `Connect`, a capped write stalled on a full socket then resumed on writable readiness. Each call is capped because Winsock takes a whole `send` request whatever `SO_SNDBUF` says, and never returns a short write: the option bounds standing buffered bytes, not one call. On Linux and macOS a connect to a listener with a full accept queue fails `Connect` of kind `TimedOut` within bound (Winsock refuses instead).
