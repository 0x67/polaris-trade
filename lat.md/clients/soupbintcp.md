...
Receive lands bytes straight into the decode buffer's spare capacity through `StreamRecv::recv_into`; the buffer advance relies on that `unsafe` trait's length contract, cited at its one `advance_mut`. Outbound bytes queue in one buffer whose front is the resume point of a partial write, shared by both drivers so wire order holds. `next_deadline` on [[crates/clients/soupbintcp/src/client.rs#SoupBinClient]] sits in an impl with no transport bound: login deadline while authenticating, else the sooner of heartbeat send and server-silence deadlines.
