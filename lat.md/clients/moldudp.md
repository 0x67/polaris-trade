...
That contract lets a caller park on an edge-triggered `ReadySet` ([[socket#Readiness]]) after `None`. In-order datagrams drain inline, borrowed from the still-owned frame with no allocation, whatever their message count (block offsets go through one receiver-owned scratch sized for a full MTU datagram); a datagram ahead of the expected sequence becomes one shared `Arc`, and its messages wait in the [[crates/clients/moldudp/src/reassembly.rs#SequenceReassembler]]. Async `recv` and `recv_owned` exist only when legs (and the requester) implement `AsyncReady`; they spin the same path, then wait on every leg. `recv` has no timer: a pending gap is re-requested again only when a datagram wakes it, so a quiet feed needs a caller timeout (the call is cancel-safe).

A gap opens only past the highest sequence seen on any source (data or heartbeat), so datagrams landing behind an open gap never re-report it or re-stage it. [[crates/clients/moldudp/src/gap.rs#GapRequestHandler#record_missing_range]] absorbs every tracked range the new one touches or overlaps, so entries stay disjoint and non-adjacent and a multi-leg feed's one-at-a-time gaps cost one re-request, not one per message; a partial fill splits the range again. With two or more legs the arbiter's clock is read once per datagram and shared by staging, first-arrival checks and confirmation; one leg reads no clock. A staged candidate is confirmed only when a later datagram is decoded after the confirm window, so on a quiet multi-leg feed confirmation waits for the next datagram (a heartbeat counts). Detection on one leg, confirmation on several, and a heartbeat tail gap each log one `warn`.
