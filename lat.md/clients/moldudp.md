...
A gap opens only past the highest sequence seen on any source (data or heartbeat), so datagrams landing behind an open gap never re-report it or re-stage it. With two or more legs the arbiter's clock is read once per datagram and shared by staging, first-arrival checks and confirmation; one leg reads no clock. A staged candidate is confirmed only when a later datagram is decoded after the confirm window, so on a quiet multi-leg feed confirmation waits for the next datagram (a heartbeat counts). Detection on one leg, confirmation on several, and a heartbeat tail gap each log one `warn`.
